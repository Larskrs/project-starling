...
│  └─ timelineSockets.ts  /timeline namespace: transport clock, relays, timelineRelay
...
| Trigger | Logs |
| --- | --- |
| Joining the room (socket `j`, after the access check) | `open` on the timeline — the primary "opened a timeline" signal |
...
The wire is **protocol 2** (`PROTOCOL` in `@starling/realtime`): one- or two-letter event names, positional arrays for fixed shapes, and rows without what the receiver can fill back in. Server code builds payloads only through the package's encoders and clients read them only through its decoders, so application code keeps readable types. See [REALTIME.md §3](REALTIME.md) for why.

| Direction | Event | Behavior |
| --- | --- | --- |
| handshake | `auth: { protocol }` | Required. `createLiveRoom` refuses any number but `PROTOCOL` **before authentication** (an outdated device costs no token lookup), with `connect_error` message `errors.protocol.unsupported` and `data: { protocol }` naming the server's, so the client can say which side is out of date instead of misreading every event. Socket.IO does not retry a refused handshake. A handshake with no `protocol` is a protocol-1 client and is refused the same way. |
| C→S | `j` `timelineId` + ack | `resolveTimelineAccess` check: timeline→production→company; allowed if global admin, company owner/admin, or production member (mirrors §5 layers 1–2 + membership). The member's role permission bits are resolved in the same pass and cached on the socket. Ack `{ ok: true, protocol, canEdit, canRename, users, anchor?, sync? }` — the occupant list, the anchor while the room plays, the resync while one runs, so a join needs no follow-up messages — or `{ error: 'Access denied' … }`. On success: join room, add to presence, and record an `open` activity row for the timeline (fire-and-forget — see §6 "Activity & recents"). |
| C→S | `l` | leave room + presence (also on `disconnect`) |
| S→C | `u` | `PresenceWire[]` — `[id, name, avatarImageId?, createdAtEpochSeconds?]` (trailing elements left out when empty; a token has neither), deduped per user across tabs. **Coalesced**: sent `PRESENCE_SETTLE_MS` (250ms) after the member set changes, only to sockets that do not already hold that version — a joiner's list is in its ack. A user's second tab arriving or leaving changes nobody's list and sends nothing. |
| S→C | `ca` / `cu` / `cd` | A clip created (the row without null columns, `createdAt` or `updatedAt`), changed (`{ id, ...the columns the request wrote }`, nulls kept — a null is the change), removed (the id). Emitted only by the **server**, via `timelineRelay` in the REST route that persisted the change, to the room except the socket named in the request's `x-socket-id` header. Clients no longer relay by hand, and the namespace accepts no client relays. |
| S→C | `ta` / `tu` / `td` / `to` | The same for tracks. A created track keeps `createdAt` and `timelineId` (the editor sorts by one and builds URLs from the other). `to` carries the full order the reorder applied (index = sortOrder). |
| C→S | `x` | `[0]` pause, `[1, frame]` play, `[2, frame]` seek — join-level (any joined member drives the shared transport). `play` requires `frame` (the sender's position becomes the shared one); `seek` requires `frame` and is accepted **only while playing** (a stopped timeline is browsed privately). The **room** takes at most one seek per 80ms, from all senders together: one arriving sooner waits for the window and a newer one replaces it, so the last frame of a scrub always lands. `pause` is idempotent and its frame is computed **from the server clock**, never taken from the client. The timeline's `frameRate` comes from the DB at join — clients cannot drive the clock with a fake fps. |
| S→C | `a` | `[playing (0\|1), frame, at, frameRate]` — the room's **authoritative transport anchor**, sent to the whole room *including the sender* after every accepted command, and in a joiner's ack while playing. `frame` is the position at server time `at`; while playing the position at any server time t is `frame + (t − at)/1000 × frameRate` — **the server decides how fast frames go**. `at` is on the server's monotonic clock (see `p`). Clients keep `at` exactly as sent and PREDICT the current frame through their *current* server-clock estimate every time they read it — never a local time converted once on arrival, which would freeze that moment's estimation error into the anchor — so a command's network delay cancels out and every client lands on the same frame. Kept per room in `roomTransport` (anchors never go stale); cleared when the room empties; clients treat a run predicted past the timeline's end as ended. `frame` is rounded to a thousandth of a frame and `at` to a tenth of a millisecond before it is stored and sent. That is all the server keeps for a playing room: it does **not** walk clip boundaries or announce which clip is live, so a play, seek or edit costs no query and no timer. Clients work the live clip out from the clips they hold and the anchor. |
| C→S | `p` | ack-only NTP-style probe: acks the server's clock in ms, rounded to a tenth (`serverNow()` in `lib/clock.ts` — monotonic, epoch-like, **not** wall-clock time; transport `at` uses the same clock). Clients estimate `serverNow = localMonotonicNow + offset`: a 5-ping burst on every (re)connect, after the WebSocket upgrade and on wake, one ping every 15s; the lowest-RTT sample of the last 2 minutes wins (`serverNow + rtt/2 − t2`, error ≤ rtt/2), samples contradicting a newer one are discarded, and changes > 40ms step while smaller ones slew at ≤ 5ms/s. See `transportClock.ts` and [integrations/protocol.md](integrations/protocol.md#server-time). |
| C→S | `r` + ack | No payload. Starts a **room-wide clock resync** (join-level, like the transport it protects). Every socket in the room is asked to re-measure. Ack `{ ok: true, requestId }` (an 8-hex-digit run id); pressing again during a run joins that run instead of starting another. Rules in `lib/clockResync.ts`. |
| S→C | `m` | `[requestId, deadlineMs]` to the whole room, the requester included: run a fresh ping burst now and answer with `e` within `deadlineMs` (4000). |
| C→S | `e` | `[requestId, rtt]` — the round trip of the sample the client's estimate rests on (offset good to `rtt/2`), or `null` if no ping answered. Reports for another run, from a socket that was not asked, or repeated, are ignored. |
| S→C | `s` / `sp` | `s` is the run whole, sent as it starts and in the ack of a socket joining mid-run: `[requestId, done, playHeld, requestedByName, deadlineMs, clients]`, one `[id, name, state, rtt?]` per socket, `state` an index into `waiting · synced · failed · no-report · left`. After that `sp` carries only what changed — `[requestId, done, playHeld, [[index, state, rtt?]…]]` — gathered for up to `RESYNC_PROGRESS_MS` (250ms); a held Play, a cancelled one and the end of the run go out at once, the end always before the released Play's anchor. **While measuring, an `x` play is held** (the newest wins, pause cancels it) and started on a fresh anchor when the run ends: every client answered, left, or the deadline passed. |
