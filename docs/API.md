...
| S→C | `playhead:sync` | `{ frame, isPlaying, userId, at }` — the relay of the above with a **server timestamp** (`at`, epoch ms at relay). Receivers age the state (`ageMs` via measured clock offset) and, while playing, apply `frame + age×fps` — landing where the sender's playhead **is now**, not where it was when the message left. The web client corrects any drift > 0.25 frames (broadcast every 500ms while playing) but restarts audio only for corrections ≥ 10 frames — smaller ones shift the audio-clock anchor so voices play on uninterrupted. The server keeps a **per-room transport state object** (`roomTransport`: `{ frame, isPlaying, userId, at }` keyed by timeline id) updated on every relay; joiners are replayed **only a playing state < 5s fresh** — the shared transport is the *active* (playing) timeline, a stopped timeline is browsed privately (the web client doesn't broadcast stopped-state seeks, and receivers ignore remote stops when already stopped). The state clears when the room empties. |
| C→S | `time:ping` | ack-only NTP-style probe: acks `Date.now()` (server epoch ms). The client runs a 5-sample burst on every (re)connect and keeps the lowest-RTT sample's offset (`serverNow + rtt/2 − localNow`) to age `playhead:sync` stamps. |
