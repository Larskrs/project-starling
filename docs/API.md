...
| C→S | `time:ping` | ack-only NTP-style probe: acks the server's clock in ms, rounded to a tenth (`serverNow()` in `lib/clock.ts` — monotonic, epoch-like, **not** wall-clock time; transport `at` uses the same clock). Clients estimate `serverNow = localMonotonicNow + offset`: a 5-ping burst on every (re)connect, after the WebSocket upgrade and on wake, one ping every 15s; the lowest-RTT sample of the last 2 minutes wins (`serverNow + rtt/2 − t2`, error ≤ rtt/2), samples contradicting a newer one are discarded, and changes > 40ms step while smaller ones slew at ≤ 5ms/s. See `transportClock.ts` and [integrations/timing.md](integrations/timing.md). |
