...
# A moving camera timeline for the multiview

A strip of the camera script, scrolling past a fixed playhead as the show runs,
made to sit along the top or bottom of a multiview. The director sees the camera
on air, the camera coming next with a countdown, and the next thirty seconds of
shots coming towards the line, without looking away from the pictures.

```
┌────────┬────────┬───────────────────────────────────────────────────────────────┐
│ ON AIR │ NEXT   │ 00:04:05      00:04:10   ┃00:04:12:08┃     00:04:15    00:04:20 │
│   C2   │ C3  6s │░░ C1 Wide ░░│░ C2 Close on┃Anna        │ C3 Crane up │ C2 Two  │
└────────┴────────┴───────────────────────────────────────────────────────────────┘
```

It is a web page, drawn on a canvas, so it goes wherever a web page can: a
browser source in OBS, a web browser input in vMix, the HTML producer in
CasparCG, or a full-screen browser on a small PC feeding a spare switcher input.
It changes nothing on the timeline, so its token only needs `VIEW`.
...
The strip reads the same camera track as the
[camera switcher](./camera-switcher.md), by the same rules, so the strip and a
switcher running beside it always agree:

- **Each block is a clip with a camera**, from its position to its end or to the
  next clip. It is coloured from the camera's hue in OKLCH, the same way the
  editor colours its clips, so a camera is the same colour on the strip, in the
  editor, and on every screen.
- **On air** is the camera of the last clip that has started. A gap between
  clips, or a clip with no camera, leaves the last camera up.
- **Next** is the next clip with a *different* camera. A new clip on the camera
  already up is not a cut, so it is not counted as one.
- **The past is dimmed**, left of the playhead. The shot on air has a white edge.
...
```sh
node --env-file=.env camera-display.ts
```

Then add `http://<display machine>:8081/` to the multiview as a web page source,
as wide as the multiview and 100 to 150 pixels tall, and place it at the top or
the bottom. The strip scales to whatever size it is given.

Settings go on the address:

| Setting | Default | What it does |
| --- | --- | --- |
| `seconds` | `30` | How many seconds of the script fit across the strip |
| `playhead` | `0.2` | Where the playhead sits, from `0` (left) to `1` (right) |
| `camera` | — | Grey out every camera but this one, such as `camera=C2`, for an operator's monitor |
| `transparent` | — | No background, for keying over the pictures |

`http://display.local:8081/?seconds=60&playhead=0.25&transparent` shows a minute
of script with the playhead a quarter of the way in, over a clear background.

---

## How it stays smooth

The service holds the connection to Cino and sends every open strip two kinds
of update:

- **The script**, whenever a clip or track changes.
- **The playhead**, on every play, pause and seek, when the clock is measured,
  and once a second besides.

Between playhead updates, the page moves the playhead on its own clock at the
timeline's frame rate, and redraws on every screen refresh. When an update
arrives within half a frame of where the page already has the playhead, the page
keeps going. Anything further, such as a seek, it jumps to.

On a local network that keeps the strip within a few milliseconds of the room,
which is plenty for a picture. It is not a clock to fire equipment from; for
that, use the SDK's clip events directly, as the switcher does.
...
**Stopped.** The strip holds still at the room's playhead, which turns grey with
*STOPPED* beside the timecode. A stopped timeline is browsed privately in the
editor, so the strip does not follow somebody scrubbing around.

**A seek.** The strip jumps to the new position at once rather than sliding
there, so it never shows a script that is not playing.

**An edit.** When someone moves, adds or recolours a clip, every strip redraws
with it within a moment, countdown included.

**Joining mid-show.** A strip opened halfway through shows the right place at
once, because everything is worked out from the playhead.

**A lost connection.** A red note appears in the corner and the strip keeps
running from the last known script and playhead. When the connection returns,
the SDK fetches the timeline again and the strip catches up.

---

## Taking it further

**More lanes.** Draw a second track under the cameras, such as the script or the
lighting cues, by sending its clips alongside the shots and giving each lane a
share of the height.

**Tally on the operators' monitors.** `?camera=C2` greys out every other camera,
so each operator's return feed can carry a strip that shows only their shots.

**Share a connection.** The strip and the [switcher](./camera-switcher.md) can
run in one process on one `live`: put both sets of listeners on it.
