...
export default defineEventHandler(async (event) => {
  ...
  const { company_id, name, parent_id } = await readValidatedBody(event, schema);
  ...
  const [folder] = await db.insert(storageFolders).values({ companyId: company_id, name, parentId: parent_id }).returning();
  return { folder };
});
