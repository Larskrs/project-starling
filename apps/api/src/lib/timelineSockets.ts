...
import {
  ...
  isClipChange, isTrackChange, isTransportCommand,
  ...
  live = createLiveRoom<TimelineCaps, PresenceUser>(io, {
    ...
    events(socket, ctx) {
      ...
      socket.on(TimelineEvent.transportCommand as never, ((raw: unknown) => {
        ...
        ctx.emit(timelineId, TimelineEvent.transportState, next);

        if (next.playing) void armWatcher(ctx.nsp, timelineId).catch(() => {});
        else disarmWatcher(timelineId);
      }) as never);

      socket.on(TimelineEvent.timePing as never, ((ack: unknown) => {
        if (typeof ack === 'function') (ack as (n: number) => void)(Date.now());
      }) as never);
