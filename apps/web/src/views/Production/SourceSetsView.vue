...
<template>
  <ManageListPage
    ...
    :description="$t('sourceSets.description', { production: data?.production?.name })"
    :loading="loading"
    :error="error"
    :count="sourceSets.length"
    :delete-target="deleteTarget"
    :deleting="deleting"
    :delete-title="$t('sourceSets.confirmDeleteTitle')"
    :delete-label="$t('sourceSets.delete')"
    :cancel-label="$t('sourceSets.cancel')"
    :delete-message="$t('sourceSets.confirmDelete', { name: deleteTarget?.name })"
  ...
  >
    ...
    <template #dialogs>
      <SourceSetDialog :open="createOpen" @update:open="createOpen = $event" @created="onCreated" />
      <SourceSetDialog
