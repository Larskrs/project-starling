 ...
 *
 *     node apps/web/src/views/TimelineEditor/useAudioEngine.test.ts
 *
...
class FakeSource extends FakeNode {
  ...
  _started = false
  _stopped = false
  constructor(ctx: unknown) { super(); this.ctx = ctx }
...
/** The decoded stand-in the fake context hands back. */
interface FakeBuffer { id: unknown; duration: number }
...
class FakeAudioContext {
  ...
  decodeAudioData(ab: { id: unknown }) {
    return new Promise<FakeBuffer>(res => pendingDecodes.push(() => res({ id: ab.id, duration: 60 })))
  }
  resume() { this.state = 'running'; return Promise.resolve() }
  close() { return Promise.resolve() }
}

// The fakes implement only what the engine touches; the casts keep the real
// global signatures without hand-writing the whole Web Audio surface.
...
function report(label: string, expected: number): boolean {
  const n  = live().length
  const ok = n === expected
...
allOk = report('seek during in-flight decode', 1) && allOk
...
allOk = report('clip edit during in-flight decode', 1) && allOk
