...
export type DownloadStatus = 'downloading' | 'decoding' | 'ready' | 'error'

/** What a file is, for the island's icon. */
export type DownloadKind = 'file' | 'image' | 'audio'
...
/** What can be known about a file before anything downloads it. */
export interface DownloadMeta {
  name?: string
  kind?: DownloadKind
}

const _files      = new Map<string, DownloadEntry>()   // fileId → live entry (mutated in place)
const _meta       = new Map<string, DownloadMeta>()    // known before a download starts
const _objectUrls = new Map<string, Promise<string>>() // cacheKey → blob: URL
...
function _notify(immediate = false): void {
...
function _schedulePrune(delayMs: number): void {
...
function _ensure(fileId: string, meta: DownloadMeta): DownloadEntry {
 ...
 */
export function describeDownload(fileId: string, { name, kind }: DownloadMeta = {}): void {
  if (!fileId) return
...
export function downloadEntry(fileId: string): DownloadEntry | null {
  const entry = _files.get(fileId)
  return entry ? { ...entry } : null
}

/** Moves a file to a later stage of the same request (e.g. bytes in → decoding). */
export function markDownloadStage(fileId: string, status: DownloadStatus): void {
  const entry = _files.get(fileId)
...
export function markDownloadDone(fileId: string): void {
  const entry = _files.get(fileId)
  if (!entry) return
  entry.status     = 'ready'
  entry.error      = null
...
export function markDownloadFailed(fileId: string, message?: string): void {
  const entry = _files.get(fileId)
  if (!entry) return
  entry.status     = 'error'
...
export function dismissDownload(fileId: string): void {
  const entry = _files.get(fileId)
...
export function dismissFinishedDownloads(): void {
  let changed = false
  for (const [fileId, entry] of _files) {
 ...
 */
export async function fetchTracked(fileId: string, url: string, meta: DownloadMeta = {}): Promise<ArrayBuffer> {
...
export function getMediaObjectUrl(
  fileId: string,
  { quality = null, name = '', kind = 'image' }: ObjectUrlOptions = {},
): Promise<string> {
  const key = quality == null ? fileId : `${fileId}@${quality}`
  const cached = _objectUrls.get(key)
  if (!cached) {
    ...
    const promise = fetchTracked(fileId, url, { name, kind })
      ...
      .catch((err: unknown) => { _objectUrls.delete(key); throw err })
...
export function resetDownloads(): void {
  for (const promise of _objectUrls.values()) {
    promise.then(url => URL.revokeObjectURL(url)).catch(() => { /* never resolved */ })
  }
